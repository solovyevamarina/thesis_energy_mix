...
def naive_forecast(country_data, start_time, end_time):
    ...
    # Directly use the existing 'Year-Month' column for easy selection
    ...
    # Loop for generating Naive forecast based on the previous year's mean
    for rep in range(12, len(ym_list)):  # Skip the first 12 months
        country_data.loc[country_data['Year-Month'] == ym_list[rep], 'Naive'] = round(
    ...
    for rep in range(start_year, end_year + 1):  # From start_year to end_year
        # Split data by year
        train_start = country_data.index[country_data['Year-Month'] == f'{rep}-01'][0]
        val_end = country_data.index[country_data['Year-Month'] == f'{rep}-12'][-1]
